...
- **Host/plugin field separation is enforced, not advisory:** the host never writes to a node's `Status`, and a plugin-supplied `Branch.State = stale` or `Truncated` is rejected/ignored — `Branch` is host-only, `Status` is plugin-only.

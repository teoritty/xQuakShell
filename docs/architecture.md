...
- **Strength meter.** `evaluatePasswordStrength` is an entropy approximation with pattern penalties, computed in the renderer with no dependencies and no I/O. It is **advisory**: a weak-but-long-enough password still creates a vault, with a warning. Only the length floor blocks.

...
- **No new privilege for the plugin process itself.** The plugin still runs out-of-process, under the same per-OS job objects and process groups (`process_sandbox.go`). The channel bus changes *what data can flow*, never *what the plugin process itself can touch on disk or network directly*. (Those limits bound resources and lifetime, not syscalls; "sandboxed", as this bullet originally put it, overstated them. The boundary and its limits are stated in [security-model.md — Trust model](../security-model.md#trust-model-and-what-it-does-not-cover). The argument here is unaffected: the point is that the channel bus adds no privilege, whatever the baseline is.)

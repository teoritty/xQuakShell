...
- **Capabilities — the provided surface.** Each capability (`network`, `filesystem`, `events`, `vault`, `session`, `auth`, `tunnel`, `channel`, `discovery`) carries its **own** semver plus named **feature flags**, defined in the capability registry ([`internal/domain/plugin/api_registry.go`](../../internal/domain/plugin/api_registry.go)).

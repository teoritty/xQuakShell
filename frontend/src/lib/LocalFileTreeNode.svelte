<script lang="ts">
  ...
  export let tree: Map<string, LocalNode[]>;
  ...
  export let onDragStartFile: (e: DragEvent, n: LocalNode) => void;
  export let onContextMenu: ((e: MouseEvent, n: LocalNode) => void) | undefined = undefined;
...
<div class="tree-node">
  ...
  >
    ...
    {/if}
    {#if showOwner}
      <span class="node-col owner">{node.owner || '—'}</span>
    {/if}

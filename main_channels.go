...
func newChannelResolverFor(audit domainplugin.ChannelAuditRecorder, embedSink usecase.EmbedFrameSink) func(domainplugin.InstalledPlugin, string) capability.ChannelBackendResolver {
	return func(plugin domainplugin.InstalledPlugin, _ string) capability.ChannelBackendResolver {
		...
		return func(purpose string) (domainplugin.ChannelPurposeBackend, error) {
			...
			case domainplugin.PurposeEmbedStream:
				return usecase.NewChannelEmbedBackend(pluginID, hasEmbedCap, embedSink, audit), nil
			default:
